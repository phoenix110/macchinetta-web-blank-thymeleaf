...
grep "<artifactId>" pom.xml | head -1 | grep -E "noorm" >/dev/null
if [ $? -eq 0 ]; then
...
sed -i -e "s|<artifactId>macchinetta-web-blank-noorm-thymeleaf</artifactId>|<artifactId>projectName</artifactId>|g" pom.xml
...
sed -i -e "s/projectName/macchinetta-web-blank-noorm-thymeleaf/g" pom.xml
